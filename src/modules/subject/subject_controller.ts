...
import { ISubject } from './subject_models.js';
...
export async function getUsersFromSubject(req:Request, res:Response): Promise<Response>{
    try{
        const name = req.params.name;
        console.log("Getting the users of the subject: " + name);
        ...
        if(users && users.length !== 0){
            ...
            return res.status(201).json({message:"There are no users",});
        }
        else
        {
            return res.status(404).json({message:"Subject not found",});
        ...
        console.log("Error finding the users of the subject " + error);
        return res.status(500).json({ error: 'Failed in getting the users' });
    }
}